...
export default Ember.Controller.extend({
  ...
  commonUpload: function (type, url, file) {
    let self = this
    ...
    ajaxUploadFile({
      ...
      onSuccess(response) {
        ...
        if (data.Status === 'success') {
          if ('script' == type) {
            self.set('model.task.Script', data.Content)
          }else {
            self.set('model.task.SrcFile', data.Content.result)
          }
        } else {
          Ember.$.notify({
  ...
  actions: {
    ...
    toggleModalScript: function () {
      ...
      if (scriptModal != true) {
        ...
        if (selectedDevices.length <= 0) {
          ...
          Ember.set(this.get('model'), 'selectedDevices', selectedDevices)
        }
      }
      this.toggleProperty('model.scriptModal')
    },
    toggleModalFile: function () {
      ...
      this.toggleProperty('model.fileModal')
    },
    ...
    uploadScript: function (file) {
      this.commonUpload('script', '/api/osinstall/v1/task/script/upload', {
        'files': file
      })
    },
    uploadFile: function (file) {
      this.commonUpload('file', '/api/osinstall/v1/task/file/upload', {
        'files': file
      })
    },
    scriptTaskAction: function () {
      this.send('commonTaskAction', 'script', 'toggleModalScript')
    },
    fileTaskAction: function () {
      this.send('commonTaskAction', 'file', 'toggleModalFile')
    },
    commonTaskAction: function (taskType, toggleModal) {
      let self = this,
      ...
      // TODO 此处需要添加脚本
      let extend = {
        'SrcFile': model.task.SrcFile,
        'DestFile': model.task.DestFile,
        'ScriptType': 'shell',
        'Script': model.task.Script,
        'ScriptParam': model.task.ScriptParam
      }

      let submitObj = {
        'TaskName': model.task.TaskName,
        'TaskChannel': model.task.TaskChannel,
        'TaskType': taskType,
        'Runas': model.task.Runas,
        'Timeout': parseInt(model.task.Timeout, 10),
        'SNs': sns,
        'Password': model.task.Password,
        'AccessToken': '',
        'Extend': extend
      }
