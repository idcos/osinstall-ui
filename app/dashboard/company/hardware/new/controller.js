...
export default Ember.Controller.extend({
    ...
    companyChange: function() {
        var self = this;
        var company = this.get('model.info.Company');
        if(!Ember.isEmpty(company)){
            var data = this.get("model.companyData");
            var tpl = this.get("model.info.Tpl");
            for(var i=0;i<data.length;i++){
            	if(company === data[i].name){
            		tpl = tpl.replace(/yourdir/ig,data[i].id);
            		set(this,"model.info.FormatTpl",$.parseJSON(tpl));
            	}
            }
        }
    }.observes("model.info.Company"),
